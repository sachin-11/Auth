...
const isAuthorized = (users) => async (req, res, next) => {
  ...
  if (isVerify) {
    if (users.indexOf("user") > -1 && req.user.role == "user") {
      const user = await Users.findById({ _id: req.user.id });
      if (!user) {
        commonResponse.unAuthentication(res, {}, constants.USER_NOT_FOUND);
      } else {
        next();
      }
      ...
      commonResponse.unAuthentication(res, {}, constants.NOT_ALLOWED, 403);
    }
  } else {
    return commonResponse.unAuthentication(res, {}, constants.SESSION_EXPIRED);
  }
};
